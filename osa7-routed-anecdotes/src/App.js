  ...
  <div>
    ...
    <ul>
      {anecdotes.map(anecdote => 
        <li key={anecdote.id} >
          <Link to={`/anecdote/${anecdote.id}`} >{anecdote.id} - {anecdote.content}</Link>
        </li>)}
    </ul>
  </div>
)
...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(show(`Added anecdote: ${content.value}`, 10))
    history.push('/')
  }
...
const App = () => {
  ...
  //console.log("Ids: ", anecdotes.map(a => a.id), " match ", match)
  return (
    <div>
